...
class ModelArgs(BaseModelArgs):
    ...
    # index_gate_rows) rows; below that the full pool is attended (measured
    # net-faster and dense-quality, ~30K context).
...
# --------------------------------------------------------------------------- #
#
# DeepSeek-V4 replaces the residual stream with multi-HyperConnection (mHC):
# the hidden state is expanded into `hc_mult` parallel copies, mixed by a
# doubly-stochastic matrix on the Birkhoff polytope (produced by Sinkhorn
# normalization), the block is applied, and the copies are recombined. The
# Sinkhorn projection and the HyperConnection/HyperHead layers live here.
...
def _make_sinkhorn_kernel(hc: int, iters: int, eps: float):
    ...
    def row_softmax():
        ...
        for r in range(hc):
            ...
            for c in range(hc):
                out.append(
                    f"          m[{base + c}] = metal::exp(m[{base + c}] - mx); s += m[{base + c}];"
                )
...
def hc_split_sinkhorn(
    ...
    use_kernel = hc_mult <= 8 and mx.metal.is_available() and comb_log.size > 0
    if use_kernel:
