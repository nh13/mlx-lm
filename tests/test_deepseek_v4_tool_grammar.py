...
The grammar only depends on the vocab through a few token-id boundaries -- ``<``
vs the single ``</`` token, the ``｜DSML｜`` special token, and the ``<think>`` /
``</think>`` markers. ``_SyntheticTokenizer`` reproduces exactly those boundaries
over a byte-level vocab, so the tests run anywhere without the model files.
"""
...
class _SyntheticTokenizer:
    """Byte-level tokenizer with the DeepSeek DSML boundaries the grammar needs.

    The multi-char markers are single ids (``</`` distinct from ``<``; ``｜DSML｜``,
    ``<think>`` and ``</think>`` each one special token); every other character is
    encoded by its UTF-8 bytes. ``encode`` is greedy longest-match so ``</think>``
    never splits into ``</`` + ``think>``.
    """

    # ordered longest-first for greedy matching
    _SPECIALS = ["</think>", "<think>", "｜DSML｜", "</", "<"]

    def __init__(self) -> None:
        self._tok_to_id = {s: i for i, s in enumerate(self._SPECIALS)}
        self._id_to_str = {i: s for s, i in self._tok_to_id.items()}
        self._byte_base = len(self._SPECIALS)
        self._size = self._byte_base + 256
    ...
    def decode(self, ids):
        ...
        for i in ids:
            if i in self._id_to_str:
                out += self._id_to_str[i].encode("utf-8")
            else:
                out.append(i - self._byte_base)
        return out.decode("utf-8", errors="replace")


_TOK = _SyntheticTokenizer()
...
class TestGrammarForcedOutput(unittest.TestCase):
    ...
    def test_required_mode_parses(self):
        ...
        out = _drive(g, _TOK.encode("dummy prompt"), "read", READ_PARAMS)
    ...
    def test_await_think_defers_forcing_until_think_close(self):
        ...
        vocab = len(_TOK)
        tokens = _TOK.encode("reason first") + [g.think_start_id]
        proc(mx.array(tokens), mx.zeros((1, vocab)))
        self.assertEqual(proc._phase, "await_think")
        for t in _TOK.encode(" thinking..."):
            tokens.append(t)
            proc(mx.array(tokens), mx.zeros((1, vocab)))
            self.assertEqual(proc._phase, "await_think")
    ...
    def test_chat_mode_forces_immediately(self):
        ...
        proc = g.logits_processor()
        vocab = len(_TOK)
        tokens = _TOK.encode("answer") + [g.think_end_id]
        proc(mx.array(tokens), mx.zeros((1, vocab)))
        self.assertEqual(proc._phase, "force")
